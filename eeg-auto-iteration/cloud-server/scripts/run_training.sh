#!/bin/bash
###############################################################################
# EEGTokenizer 自动回环训练脚本（云服务器版本）
# 用法：./run_training.sh <训练命令>
#
# 功能：
...
if [ $# -lt 1 ]; then
    ...
    echo "  $0 \"cd ~/EEGTokenizer && python train.py --epochs 100 --lr 0.001\""
    echo ""
    echo "环境变量："
    echo "  NIT_SSH_PORT=3022         nit 隧道端口（默认 3022）"
    echo "  AUTO_SYNC=false          跳过代码自动同步（默认 true）"
    echo "  WEBHOOK_URL=...          自定义 webhook 地址"
    exit 1
fi
...
REMOTE_CMD="
    ...
    { ${TRAINING_CMD}; } > ${REMOTE_LOG_FILE} 2>&1
    echo \$? > /tmp/exit_code_${RUN_ID}
"

# 执行远程命令
set +e
ssh -p "${NIT_SSH_PORT}" localhost "${REMOTE_CMD}" 2>&1 | tee -a "$RUN_LOG_FILE"
...
# Step 3: 获取远程日志
log_step "Step 3/3: 获取远程日志..."

# 下载远程日志文件
scp -P "${NIT_SSH_PORT}" "localhost:${REMOTE_LOG_FILE}" "$RUN_LOG_FILE.full" 2>/dev/null || true
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    # 下载错误日志
    ssh -p "${NIT_SSH_PORT}" localhost "cat ${REMOTE_LOG_FILE}" 2>/dev/null > "$ERROR_LOG_FILE" || true
